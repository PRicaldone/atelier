...
export class AtelierRoutineAgent {
        ...
        if (check.automated) {
          ...
          switch (check.id) {
            ...
            case 'connectors-health':
              ...
              break;
            case 'smart-folded-compliance':
              result = await this.checkSmartFoldedCompliance();
              break;
            case 'smart-folded-deep-audit':
              result = await this.checkSmartFoldedDeepAudit();
              break;
            case 'code-quality':
  ...
  /**
   * Smart Folded Text Block Compliance Check (Daily)
   * Verifies all text blocks have titles and follow folding patterns
   */
  async checkSmartFoldedCompliance() {
    const checkName = 'Smart Folded Text Block Compliance';
    this.logger.info(`🔍 Executing: ${checkName}`, 'checkSmartFoldedCompliance');

    try {
      const results = [];
      const violations = [];

      // Check Scriptorium/Canvas text blocks
      if (typeof window !== 'undefined' && window.useCanvasStore) {
        const canvasState = window.useCanvasStore.getState();
        const textElements = canvasState.elements?.filter(el => el.type === 'note' || el.type === 'text') || [];
        
        textElements.forEach(element => {
          const hasTitle = element.name || element.title || element.text?.split('\n')[0];
          const hasContent = element.text || element.content;
          ...
          if (hasContent && hasContent.split('\n').length > 4 && !element.smartFolded) {
            violations.push({
              module: 'scriptorium', 
              elementId: element.id,
              issue: 'Long content without smart folding',
              severity: 'warning'
            });
          }
        });
        ...
        results.push({
          test: 'Scriptorium Text Elements',
          status: violations.filter(v => v.module === 'scriptorium' && v.severity === 'critical').length === 0 
            ? AgentStatus.HEALTHY : AgentStatus.CRITICAL,
          details: `${textElements.length} text elements, ${violations.filter(v => v.module === 'scriptorium').length} issues`,
          data: { total: textElements.length, violations: violations.filter(v => v.module === 'scriptorium') }
        });
      }

      // Check Mind Garden text nodes
      if (typeof window !== 'undefined' && window.useMindGardenStore) {
        const mindGardenState = window.useMindGardenStore.getState();
        const textNodes = mindGardenState.nodes?.filter(node => node.type === 'card' || node.data?.type === 'text') || [];
        ...
        textNodes.forEach(node => {
          ...
          if (hasContent && hasContent.split('\n').length > 3 && !node.data?.smartFolded) {
            violations.push({
              module: 'mind-garden',
              nodeId: node.id,
              issue: 'Long content without smart folding',
        ...
        results.push({
          test: 'Mind Garden Text Nodes',
          status: violations.filter(v => v.module === 'mind-garden' && v.severity === 'critical').length === 0
            ? AgentStatus.HEALTHY : AgentStatus.CRITICAL,
          details: `${textNodes.length} text nodes, ${violations.filter(v => v.module === 'mind-garden').length} issues`,
          data: { total: textNodes.length, violations: violations.filter(v => v.module === 'mind-garden') }
        });
      }

      // Check Ideas module
      if (typeof window !== 'undefined' && window.useIdeasStore) {
        const ideasState = window.useIdeasStore.getState();
        ...
        results.push({
          test: 'Ideas Module',
          status: violations.filter(v => v.module === 'ideas' && v.severity === 'critical').length === 0
            ? AgentStatus.HEALTHY : AgentStatus.WARNING,
          details: `${ideas.length} ideas, ${violations.filter(v => v.module === 'ideas').length} issues`,
          data: { total: ideas.length, violations: violations.filter(v => v.module === 'ideas') }
        });
      }
      ...
      results.push({
        test: 'Smart Folded Compliance Summary',
        status: criticalViolations === 0 ? (warningViolations === 0 ? AgentStatus.HEALTHY : AgentStatus.WARNING) : AgentStatus.CRITICAL,
        details: `${criticalViolations} critical, ${warningViolations} warning violations`,
        data: { 
          summary: { critical: criticalViolations, warnings: warningViolations, total: violations.length },
          violations: violations
        }
      });

      // Add human actions if violations found
      if (violations.length > 0) {
        this.humanInputRequired.push({
          check: checkName,
          action: 'Fix Smart Folded Text Block violations',
          reason: `Found ${violations.length} compliance violations`,
          violations: violations,
          priority: criticalViolations > 0 ? CheckPriority.HIGH : CheckPriority.MEDIUM
        });
      }
      ...
      this.recordCheckResult(checkName, overallStatus, results, {
        ...
        needsHumanInput: violations.length > 0
      });

      return { status: overallStatus, results, violations };
      ...
      return { status: AgentStatus.CRITICAL, error: error.message };
    }
  }

  /**
   * Smart Folded Text Block Deep Audit (Weekly)
   * Comprehensive analysis of text block architecture across modules
   */
  async checkSmartFoldedDeepAudit() {
    const checkName = 'Smart Folded Deep Architecture Audit';
    this.logger.info(`🔍 Executing: ${checkName}`, 'checkSmartFoldedDeepAudit');

    try {
      const results = [];
      const architecturalIssues = [];

      // Check UCA Integration
      if (typeof window !== 'undefined' && window.__ucaIntegration) {
        const ucaContent = await window.__ucaIntegration.getAllWorkspaceContent();
        const textContent = ucaContent.filter(item => 
          item.contentType === 'text' || item.contentType === 'SMART_FOLDED_TEXT'
        );
        
        const ucaCompliantCount = textContent.filter(item => 
          item.ucaMetadata && item.aiReadable !== undefined
        ).length;

        results.push({
          test: 'UCA Integration Compliance',
          status: ucaCompliantCount === textContent.length ? AgentStatus.HEALTHY : AgentStatus.WARNING,
          details: `${ucaCompliantCount}/${textContent.length} text blocks are UCA compliant`,
          data: { compliant: ucaCompliantCount, total: textContent.length, compliance: ucaCompliantCount / textContent.length }
        });
      }

      // Check Cross-Module UX Consistency
      const uxConsistencyScore = this.analyzeCrossModuleUXConsistency();
      results.push({
        test: 'Cross-Module UX Consistency',
        status: uxConsistencyScore >= 0.9 ? AgentStatus.HEALTHY : (uxConsistencyScore >= 0.7 ? AgentStatus.WARNING : AgentStatus.CRITICAL),
        details: `${Math.round(uxConsistencyScore * 100)}% consistency between Scriptorium ↔ Mind Garden`,
        data: { consistencyScore: uxConsistencyScore }
      });

      // Check Smart Folded Component Usage
      if (typeof window !== 'undefined' && window.__ucaComplianceChecker) {
        try {
          const modules = {
            scriptorium: window.useCanvasStore?.getState(),
            mindGarden: window.useMindGardenStore?.getState(),
            ideas: window.useIdeasStore?.getState()
          };

          const complianceReport = window.__ucaComplianceChecker.generateFullUCAReport(modules);
          
          results.push({
            test: 'UCA Compliance Report',
            status: complianceReport.overallCompliance ? AgentStatus.HEALTHY : AgentStatus.WARNING,
            details: `${complianceReport.summary.compliant}/${complianceReport.summary.compliant + complianceReport.summary.nonCompliant} modules compliant`,
          ...
          results.push({
            test: 'UCA Compliance Report',
            status: AgentStatus.WARNING,
            details: 'Could not generate compliance report',
      ...
      return { status: overallStatus, results, architecturalIssues };
  ...
  /**
   * Analyze Cross-Module UX Consistency
   * Helper method for deep audit
   */
  analyzeCrossModuleUXConsistency() {
    let consistencyScore = 1.0;
    
    try {
      // Check if both modules follow same title patterns
      const canvasElements = window.useCanvasStore?.getState()?.elements || [];
      const mindGardenNodes = window.useMindGardenStore?.getState()?.nodes || [];
      
      const canvasTitleCompliance = canvasElements.filter(el => 
        el.type === 'note' && (el.name || el.title || el.text?.split('\n')[0])
      ).length / Math.max(canvasElements.filter(el => el.type === 'note').length, 1);
      
      const mindGardenTitleCompliance = mindGardenNodes.filter(node => 
        node.type === 'card' && node.data?.title
      ).length / Math.max(mindGardenNodes.filter(node => node.type === 'card').length, 1);
      
      const titleConsistency = 1 - Math.abs(canvasTitleCompliance - mindGardenTitleCompliance);
      consistencyScore *= titleConsistency;
      
      // Add other consistency checks as needed
      
    } catch (error) {
      this.logger.warn(`Error analyzing UX consistency: ${error.message}`, 'analyzeCrossModuleUXConsistency');
      consistencyScore = 0.5; // Default to moderate score on error
    }
