...
export const useCanvasStore = create(
  subscribeWithSelector((set, get) => ({
    ...
    // Contract compliance methods - required by ICanvas
    
    // Get all elements (contract method)
    getElements: () => {
      return get().elements;
    },
    
    // Get selected element IDs (contract method)  
    getSelectedIds: () => {
      return get().selectedIds;
    },
    
    // Get current board ID (contract method)
    getCurrentBoardId: () => {
      return get().currentBoardId;
    },
    
    // Save to project (contract method) - delegates to existing saveToProject
    saveToProject: () => {
      // This method already exists but wasn't exposed at top level
      // Delegate to the project saving functionality
      const state = get();
      state.saveCurrentLevelToHierarchy();
      
      // Also save to project workspace if available
      const projectStore = useProjectStore.getState();
      if (projectStore.currentProjectId) {
        try {
          const currentProject = projectStore.getCurrentProject();
          if (currentProject) {
            currentProject.workspace = currentProject.workspace || {};
            currentProject.workspace.canvas = {
              elements: state.elements,
              viewport: state.viewport,
              selectedIds: state.selectedIds,
              currentBoardId: state.currentBoardId,
            ...
            console.log('✅ Canvas data saved to project');
          }
        } catch (error) {
          console.error('❌ Failed to save canvas to project:', error);
        }
    ...
    // Zoom viewport (contract method) - adding missing method
    zoomViewport: (zoomDelta, center = null) => {
      set((state) => {
        let newZoom = Math.max(0.1, Math.min(5, state.viewport.zoom + zoomDelta));
    ...
    // Delete element (contract method)  
