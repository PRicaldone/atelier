...
const CreativeAtelier = () => {
  ...
  useEffect(() => {
    ...
    // Auto-restored by AIFeatureManager when APIs are available
    const shouldAnalyze = process.env.NODE_ENV === 'production' || 
                         window.__AI_FEATURES_ENABLED?.canvasAutoAnalysis;
  ...
  // Listen for AI feature restoration
  useEffect(() => {
    const handleAIFeatureRestored = (event) => {
      if (event.detail.feature === 'canvas-auto-analysis') {
        console.log('🤖 Canvas auto-analysis automatically restored!');
        // Trigger analysis now that it's enabled
        analyzeCanvasContext();
      }
    };
    
    window.addEventListener('ai-feature-restored', handleAIFeatureRestored);
    return () => window.removeEventListener('ai-feature-restored', handleAIFeatureRestored);
  }, [analyzeCanvasContext]);
