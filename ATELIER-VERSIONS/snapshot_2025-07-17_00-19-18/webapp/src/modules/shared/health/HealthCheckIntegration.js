...
export async function initializeHealthChecking() {
  ...
  try {
    ...
    // Register Orchestra health provider
    try {
      const orchestraModule = await moduleRegistry.getModule('orchestra');
      if (orchestraModule) {
        const orchestraProvider = new OrchestraHealthProvider(orchestraModule);
        healthCheckManager.registerModule('orchestra', orchestraProvider, {
          pingInterval: 8000, // 8 seconds
          pingTimeout: 3000,
          maxRetries: 2
        });
      }
    } catch (error) {
      healthLogger.warning('Orchestra module not available for health checking', 'initializeHealthChecking', {
        error: error.message
      });
    }
