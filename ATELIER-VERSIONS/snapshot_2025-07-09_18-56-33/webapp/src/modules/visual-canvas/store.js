...
export const useCanvasStore = create(
  subscribeWithSelector((set, get) => ({
    ...
    saveCurrentLevelToParent: () => {
      const state = get();
      console.log('saveCurrentLevelToParent - currentBoardId:', state.currentBoardId, 'elements:', state.elements.length, 'boardHistory:', state.boardHistory);
      
      if (!state.currentBoardId) {
        // We're at root, save to localStorage
        console.log('saveCurrentLevelToParent - saving to root localStorage');
      ...
      const rootElements = JSON.parse(localStorage.getItem(STORAGE_KEYS.CANVAS_ELEMENTS) || '[]');
      console.log('saveCurrentLevelToParent - loaded rootElements:', rootElements.length);
      
      const updateBoardElements = (elements, targetBoardId, newElements, path = []) => {
        console.log('updateBoardElements - searching for', targetBoardId, 'in', elements.length, 'elements, path:', path);
        
        return elements.map(element => {
          if (element.id === targetBoardId && element.type === 'board') {
            console.log('updateBoardElements - FOUND target board:', targetBoardId, 'updating with', newElements.length, 'elements');
            return {
              ...element,
              data: {
                ...element.data,
          ...
          if (element.type === 'board' && element.data?.elements) {
            console.log('updateBoardElements - recursing into board:', element.id, 'children:', element.data.elements.length);
            const updatedChildren = updateBoardElements(element.data.elements, targetBoardId, newElements, [...path, element.id]);
            return {
              ...element,
              data: {
                ...element.data,
      ...
      console.log('saveCurrentLevelToParent - saving updated rootElements:', updatedRootElements.length);
      localStorage.setItem(STORAGE_KEYS.CANVAS_ELEMENTS, JSON.stringify(updatedRootElements));
      
      // Verify the save worked correctly
      const verification = JSON.parse(localStorage.getItem(STORAGE_KEYS.CANVAS_ELEMENTS) || '[]');
      console.log('saveCurrentLevelToParent - verification:', verification.length, 'elements saved');
      
      // Verify the board we just saved can be found
      const testFind = get().findBoardInHierarchy(state.currentBoardId);
      console.log('saveCurrentLevelToParent - can find saved board:', state.currentBoardId, ':', !!testFind);
    },
