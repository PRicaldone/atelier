...
post_commit_validation() {
    ...
    for file in "${critical_files[@]}"; do
        ...
        if [[ -f "$filepath" ]] && [[ -s "$filepath" ]]; then
            ...
            log "INFO" "✓ $file local file validated (${local_size} bytes)"
            ...
            if git -C "$MAIN_REPO" show "HEAD:$file" > /dev/null 2>&1; then
                ...
                if [[ $git_size -gt 0 ]]; then
                    ...
                    log "SUCCESS" "✅ Restored $file from git after commit"
                else
                    validation_issues+=("$file: empty in both local and git")
                fi
            else
    ...
    if [[ ${#validation_issues[@]} -gt 0 ]]; then
        ...
        log "SUCCESS" "✅ Post-commit validation: All critical files consistent"
    fi
