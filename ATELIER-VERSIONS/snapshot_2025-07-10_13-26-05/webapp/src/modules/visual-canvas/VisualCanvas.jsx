...
const VisualCanvas = () => {
  ...
  const handleDragEnd = useCallback((event) => {
    ...
    if (over && over.data.current?.type === 'tree-drop-zone') {
      ...
      console.log('Dropping element', draggedElementId, 'into board', targetBoardId);
      
      // Move element to target board
      const store = useCanvasStore.getState();
      const element = store.getElementById(draggedElementId);
      
      if (element) {
        // Remove from current location
        store.removeElement(draggedElementId);
        
        // Add to target board
        store.moveElementToBoard(element, targetBoardId);
    ...
    setDraggedElement(null);
    setDragOffset({ x: 0, y: 0 });
  }, [selectedIds, viewport.zoom, moveElement, elements, settings.snapToGrid, selectElement, addElement, viewport.x, viewport.y]);
